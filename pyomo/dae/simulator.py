...
class Pyomo2Scipy_Visitor(EXPR.ExpressionReplacementVisitor):
    """
    Expression walker that replaces instances of _GetItemExpression
    and _GetAttrExpression with mutable parameters.
    """
...
class Substitute_Pyomo2Casadi_Visitor(EXPR.ExpressionReplacementVisitor):
    ...
    Expression walker that replaces
       ...
       * _GetItemExpressions and _GetAttrExpressions with _GetAttrIndexer objects that 
         reference CasADi variables.
    """
...
class Simulator:
    ...
    def __init__(self, m, package='scipy'):
        ...
        for con in has_t_con:
            ...
            # FIXME: Update discretization transformation to keep a list of
            # the discretization equations and check that instead of relying 
